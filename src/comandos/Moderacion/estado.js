...
module.exports = {
    ALIASES: ['presence'],
    DESCRIPTION: "Sirve para actualizar la presencia del bot",
    
    async execute(client, message, args, prefix){
        //comprobaciones previas
        const sub = args[0].toLowerCase()
        if (!sub) { return console.log(`❌ Debes colocar el tipo de presencia que quieres cambiar`)}

        const tipo = args[1].toLowerCase()
        if (!tipo) { return console.log(`❌ Debes colocar el tipo de estado/actividad que quieres cambiar`)}
        ...
        message.delete();
        return console.log(`Se cambió ${sub} a ${tipo}.`) 
    } 
}